...
const uploadDir = path.join(os.tmpdir(), 'FoodBridge-fssai-uploads');
...
// Create a new FoodLoop user in Supabase
// Supabase/PostgreSQL generates the UUID automatically.
// Create a new FoodLoop user in Supabase
app.post("/api/users", async (req, res) => {
    try {
        ...
        // The database automatically generates the UUID.
        const { data, error } = await supabase
            .from("users")
            .insert({
                role: user.role,
                ...
                daily_meals_required:
                    user.dailyMealsRequired ?? null,
                dietary_needs:
                    user.dietaryNeeds ?? null,

                // Waste processor information
                facility_type:
                    user.facilityType ?? null,
                ...
                certificate_uploaded:
                    user.certificateUploaded ?? false,

                extracted_fssai_number:
                    user.extractedFssaiNumber ?? null,
                certificate_expiry_date:
                    user.certificateExpiryDate ?? null,
